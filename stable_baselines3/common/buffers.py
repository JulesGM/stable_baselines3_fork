...
class RolloutBuffer(BaseBuffer):
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        """
        # Convert to numpy
        last_values = conv_bfloat16(last_values.clone().cpu()).numpy().flatten()
    ...
    def add(
        ...
        self.values[self.pos] = conv_bfloat16(value.clone().cpu()).numpy().flatten()
        self.log_probs[self.pos] = conv_bfloat16(log_prob.clone().cpu()).numpy()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
