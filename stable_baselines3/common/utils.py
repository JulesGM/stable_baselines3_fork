...
def conv_bfloat16(tensor):
    assert isinstance(tensor, (th.Tensor, list)), "Only tensors and list of tensors are supported"

    ###########################################################################
    # External object is not a tensor
    ###########################################################################
    if not isinstance(tensor, th.Tensor):
        if (tensor and isinstance(tensor, list) and 
            isinstance(tensor[0], th.Tensor) and 
            tensor[0].dtype == th.bfloat16
        ):
            tensor = [x.float() for x in tensor]
        return tensor

    ###########################################################################
    # External object is a tensor
    ###########################################################################
    if tensor.dtype == th.bfloat16:
        return tensor.float()
